...
public function index(ReponseRepository $reponseRepository, ReclamationRepository $reclamationRepository): Response
{
    ...
    // Try to find a reclamation that doesn't have a response yet
    $reclamationsWithoutResponses = $reclamationRepository->findReclamationsWithoutResponses();
    $reclamation = !empty($reclamationsWithoutResponses) ? $reclamationsWithoutResponses[0] : null;
    
    // If no reclamations without responses, just get the first one
    if (!$reclamation) {
    $reclamation = $reclamationRepository->findOneBy([], ['id' => 'ASC']); 
    }

...
class PatientRDVController extends AbstractController
{
    ...
    public function prendreRendezVous(
        ...
        NotificationService $notificationService // Ajoutez le service de notification ici
    ): Response {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                $notificationService->createNotificationForMedecin(
                    $medecin,
                    'Un nouveau rendez-vous a été demandé par ' . $patient->getUser()->getFirstName() . ' ' . $patient->getUser()->getLastName() . 
                    ' pour le ' . $rendezVous->getDate()->format('d/m/Y') . ' à ' . $rendezVous->getHeure()->format('H:i') . '.'
                );
    ...
    public function createNotificationForMedecin($medecin, $message)
    {
        // Logique pour créer et enregistrer la notification pour le médecin
   ...
   private function filterEtatChoicesByMedecin(Medecin $medecin, EtatRendezVousRepository $etatRendezVousRepository): array
{
    ...
    // Récupérer les types de rendez-vous du médecin
    $typesRendezVousMedecin = $medecin->getTypesRendezVous();

    // Filtrer les états selon les types de rendez-vous du médecin
    $filteredEtats = array_filter($allEtats, function ($etat) use ($typesRendezVousMedecin) {
        return in_array($etat->getId(), $typesRendezVousMedecin);
    });

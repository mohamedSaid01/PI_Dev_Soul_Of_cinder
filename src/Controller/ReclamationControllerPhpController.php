...
final class ReclamationControllerPhpController extends AbstractController
{
    ...
    public function indexcopy(ReclamationRepository $reclamationRepository, EntityManagerInterface $entityManager): Response
    {
        // Get all reclamations
        $allReclamations = $reclamationRepository->findAll();
        
        // Filter out reclamations with invalid medecin references
        $validReclamations = [];
        foreach ($allReclamations as $reclamation) {
            // If the reclamation has a medecin, check if it's valid
            if ($reclamation->getMedecin() !== null) {
                // Try to find the medecin in the database
                $medecinId = $reclamation->getMedecin()->getId();
                $medecin = $entityManager->getRepository(Medecin::class)->find($medecinId);
                
                // If medecin doesn't exist, set it to null
                if ($medecin === null) {
                    $reclamation->setMedecin(null);
                    $entityManager->persist($reclamation);
                }
            }
            $validReclamations[] = $reclamation;
        }
        
        // Flush changes to database if needed
        $entityManager->flush();
        
        return $this->render('reclamation_controller_php/back.html.twig', [
            'reclamations' => $validReclamations,
        ]);
    }
    ...
    public function new(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
                ...
                $this->addFlash('success', 'Votre réclamation a reçu une réponse.');
            } catch (\Exception $e) {
